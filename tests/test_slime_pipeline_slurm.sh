#!/bin/bash
#SBATCH --job-name=test_slime_pipeline
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=test_slime_pipeline_%j.out
...
SCRATCH_TEMP_DIR="/leonardo_scratch/large/userexternal/$USER/tmp_slime_pipe_$SLURM_JOB_ID"
...
# Export variables for the container environment
...
# Python mock data generation logic remains consistent with original pipeline test
...
echo "🚀 Launching SLIME Pipeline Test..."
singularity exec --nv \
...
if [ -f "$RESULT_JSON" ]; then
    echo "✅ Success: slime_summary.json verified."
else
