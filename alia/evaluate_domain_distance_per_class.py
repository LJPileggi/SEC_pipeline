...
def parsing():
    ...
    # 🎯 ADDED: Filter to lock the current process to a single isolated class execution
...
def main():
    ...
    if args.class_to_process:
        print(f"   • Tranche Isolata Attiva: Classe [{args.class_to_process}]")
    ...
    print("-> Inizio processing ad isolamento di risorsa...")
    
    # Iterating over classes: opening and closing one HDF5 container at a time
    for class_name in classes:
        # 🎯 TRANCHED FILTER: Skip execution if it does not match the target bounded class
    ...
    # Safe guard check: save results only if data was actually processed in this tranche
    if len(per_audio_results) > 0:
        df = pd.DataFrame(per_audio_results)
        output_dir = "results/domain_analysis_online"
        os.makedirs(output_dir, exist_ok=True)
        
        # 🎯 TRANCHED SAVE: Save outputs with a class suffix to prevent data collision during sequential execution
        ...
        print("\n" + "="*55)
        print(f"📊 TRANCHE PER CLASSE [{args.class_to_process}] COMPLETATA")
        print(f"   • Tracce salvate: {len(df)}")
        print(f"   • File generato:   {output_dir}/online_per_audio_distances{class_suffix}.csv")
        print("="*55 + "\n")
    else:
        print("⚠️ Nessun dato raccolto per questa tranche. Verificare i percorsi dei file.")
