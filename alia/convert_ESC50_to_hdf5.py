...
# ==============================================================================
# NUOVE FUNZIONI MODULARI: ANALISI CLASS-WISE & HUNGARIAN MATCHING
# ==============================================================================
...
def compute_semantic_silhouette(X, y_true, classes):
    ...
    Restituisce un dizionario con media e deviazione standard per classe.
    """
...
def compute_contingency_and_matching(y_true, y_pred, n_classes):
    ...
    """
    # Matrice di contingenza N: righe = true class, colonne = cluster
    N = np.zeros((n_classes, n_classes), dtype=int)
    for t, p in zip(y_true, y_pred):
        if 0 <= t < n_classes and 0 <= p < n_classes:
            N[t, p] += 1

    # Hungarian matching per massimizzare la traccia (equivalente a minimizzare -N)
    row_ind, col_ind = linear_sum_assignment(-N)
    
    # Mappa cluster -> classe true abbinata
    cluster_to_class = {c: r for r, c in zip(row_ind, col_ind)}
    
    # Assegna le label matchate a ogni campione
    y_matched = np.array([cluster_to_class.get(p, -1) for p in y_pred])
    
    # Matrice di confusione cluster-matched: M[i, j] = campioni di classe i associati al cluster della classe j
...
def compute_class_metrics_single_run(N, matched_matrix, classes):
    """
    Calcola le metriche class-wise richieste (A, B, C, D) su una singola esecuzione.
    """
    ...
    for i, c_name in enumerate(classes):
        ...
        # Nella matched_matrix: righe = true class, colonne = matched class
        ...
        # D. Principale altra classe con cui viene confusa
        confused_row = matched_matrix[i, :].copy()
        confused_row[i] = -1  # Escludiamo i campioni corretti
...
def plot_and_save_heatmap(matrix, row_labels, col_labels, title, save_path, fmt="{:.2f}", cmap="viridis"):
    """Salva una heatmap ad alta risoluzione con Matplotlib puro (senza seaborn)."""
    ...
    # Annotazioni nelle celle (se la griglia non è troppo densa)
...
def main():
    ...
    global_results = {}
    N_RUNS = 20  # 20 seed stocastici per cluster per garantire rigore statistico
    ...
    for feat_name, X in features.items():
        ...
        # 1. Pipeline di Preprocessing e PCA (invariata rispetto al paper)
        pca_full = PCA().fit(X)
        n_comp = np.argmax(np.cumsum(pca_full.explained_variance_ratio_) >= 0.9) + 1
        X_pca = PCA(n_components=n_comp).fit_transform(X)

        # 2. Semantic Silhouette (Punto 4: ground-truth geometric separability)
        ...
        for algo_name in ["kmeans", "bisecting"]:
            ...
            # Struttura per accumulare le metriche dei 20 seed
            ...
            # Per salvare le matrici rappresentative (Run 0, seed 42)
            first_N = None
            first_matched = None
            ...
            for c_name in classes:
                ...
                # Identifica la classe con cui si è confusa più frequentemente nei 20 seed
                conf_counts = {}
