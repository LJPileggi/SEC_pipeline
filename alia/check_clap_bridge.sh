...
try:
    ...
    dataset.close()
    print("📖 Audio caricato tramite DistributedAudioRAWDataset.")
except Exception as e:
    print(f"⚠️ Caricamento dataset fallback via h5py: {e}")
    ...
    print(f"📖 Audio caricato da: {sample_h5_path}")
...
with torch.no_grad():
    ...
    print(f"📐 Dimensione x_norm (post-bn0 nativo): {x_norm.shape}")
    x_0_pristine = x_norm.permute(0, 1, 3, 2)
    print(f"📐 Dimensione x_0_pristine (spazio U-Net):  {x_0_pristine.shape}")

print("\n" + "="*65)
print("🔍 TEST 2: ALLINEAMENTO PONTE CLAP (Log-Mel Nativo)")
print("="*65)

def format_for_htsat(mel):
    # reshape_wav2img richiede: [Batch, Channel, Time <= 1024, Freq == 64]
    if mel.ndim == 3:
        mel = mel.unsqueeze(1)
    if mel.shape[2] == 64 and mel.shape[3] != 64:
        mel = mel.transpose(2, 3)
    return mel

with torch.no_grad():
    mel_input = format_for_htsat(x_0_pristine)
    print(f"📐 Dimensione tensore passato a reshape_wav2img: {mel_input.shape}")

    x_ready_manual = htsat.reshape_wav2img(mel_input)
    print(f"📐 Dimensione output reshape_wav2img: {x_ready_manual.shape}")
    ...
    print(f"🎯 Similarità Coseno (Embedding Ufficiale vs Mel Nativo Iniettato): {sim_manual:.6f}")
...
if not pts:
    print("❌ Nessun checkpoint (.pt) trovato in /tmp_data/models/diff_model.")
    sys.exit(0)
